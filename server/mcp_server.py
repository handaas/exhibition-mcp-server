...
@mcp.tool()
def exhibition_bigdata_fuzzy_search(matchKeyword: str, pageIndex: int = 1, pageSize: int = 50) -> dict:
    """
...
@mcp.tool()
def exhibition_bigdata_exhibition_venue_search(pavilionRegion: str, matchKeyword: str = None, pageIndex: int = 1, pageSize: int = 10) -> dict:
    """
    ...
    - pavilionRegion: 会馆所在地区 类型：string - 会馆所在省份城市，不可多选，示例："福建省,厦门市",四个直辖市为："北京"、"上海"、"天津"、"重庆"
    - pageIndex: 页码 类型：int - 默认从1开始
...
@mcp.tool()
def exhibition_bigdata_exhibition_search(matchKeyword: str = None, pageIndex: int = 1, pageSize: int = 50) -> dict:
    """
...
def exhibition_bigdata_exhibitor_search(fairRegion: str = None, matchKeyword: str = None, fairIndustries: str = None,
                     fairBeginTimes: str = None, fairEndTimes: str = None, pageIndex: int = 1, fairFilter: str = None, pageSize: int = 10) -> dict:
    """
    ...
    请求参数:
    - fairRegion: 省市地区 类型：string - 展商所在省份/城市，不可多选，示例："福建省,厦门市",四个直辖市为："北京"、"上海"、"天津"、"重庆"
    ...
    - pageIndex: 分页开始位置 类型：默认从1开始
    - fairCount: 参展记录 类型：select - 参展记录枚举（5场以下、5-10场、10-20场、20-50场、50-100场、100场以上）,默认为""
    - pageSize: 分页结束位置 类型：int - 一页最多获取50条数据, 不能超过50, 超过50的统一用50代替
    ...
    - resultList: 结果列表 类型：list of dict
        - enterpriseInfo: 企业信息 类型：list of dict
            - operStatus: 企业状态 类型：string
            - enterpriseScaleAlgValueV2: 公司规模 类型：string
            - foundTime: 成立时间 类型：string
